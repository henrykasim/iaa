...
# /runPM/ - @get: return all previously submitted predictive model 
# /runPM/ - @post: upload data and build predictive model based on AutoML approach
class RunPredictiveModelAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'uid' not in request.data:
            uid = int(mktime(time_start.timetuple()))
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                # Check additional parameter passing
                if 'nModel' in request.data:
                    data.nModel = int(request.data['nModel'])
                    print("Number of Model specified: "+str(data.nModel))
                if 'maxRuntime' in request.data:
                    data.maxRuntime = int(request.data['maxRuntime'])
                ...
                # Run AutoML to build Predictive Model
                print('Building PM Start:', datetime.datetime.now())
...
# /pmdata/uid - @delete: remove dataset and the PM
...
# /pm/uid - @get: Display PM result detail
class PMAPIView(APIView):
    renderer_classes = (JSONRenderer, )
    def get_object(self, uid):
        try:
            return PredictiveModel.objects.get(pk=uid)
